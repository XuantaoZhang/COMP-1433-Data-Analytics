...
while(j<100){
  ...
  while(a<151){
    x=iris[a,3]
    y=iris[a,4]
    a_distance = sqrt((xa-x)**2+(ya-y)**2)
    distance_1=c(distance_1,a_distance)
    
    b_distance=sqrt((xb-x)**2+(yb-y)**2)
    distance_2=c(distance_2,b_distance)
    
    c_distance=sqrt((xc-x)**2+(yc-y)**2)
  ...
  C=1
  while(A<=A_count){
    j_sum_a=j_sum_a+(cluster_a_distance[A])**2
    A=A+1
  }
  while(B<=B_count){
    j_sum_b=j_sum_b+(cluster_b_distance[B])**2
    B=B+1
  }
  while(C<=C_count){
    j_sum_c=j_sum_c+(cluster_c_distance[C])**2
    C=C+1
  }
  ...
  #if the old centroids is not needed, change xA=xa to =0, count_a=A_count
  xA=0
  yA=0
  count_a=1
